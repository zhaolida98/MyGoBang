...
class AI(object):
    ...
    def findPattern(self, chessboard, x, y, role):
        ...
        count += counterCount
        if count == 3 and barrier ==0:
            three = three + 1
        if count >= 4 and barrier == 1:
            dead_four = dead_four + 1
        ...
        while True:
            ...
            tempPoint = chessboard[i][y]
            if tempPoint == COLOR_NONE:
                if gap == -1 and i > 0 and chessboard[i- 1][y] == role:
                    gap = 0
                    continue
                else:
                    break
        ...
        if three >= 2:
            return score['DOUBLE_THREE'] + result
        elif three == 1 and dead_four >= 1:
            return score['FOUR_THREE'] + result
        else:
        ...
        for i in range(0, boarder):
            for j in range(0, boarder):
                if chessboard[i][j] == comp_color:
                    temp_comp_score = self.findPattern(chessboard, i, j, comp_color)
                    comp += temp_comp_score
                    if i == 9 and j == 8:
                        print("comp score at 9 8 is",temp_comp_score )
                elif chessboard[i][j] == hum_color:
                    temp_hum_score = self.findPattern(chessboard, i, j, hum_color)
                    hum += temp_hum_score
                    if i == 9 and j == 8:
                        print("hum score at 9 8 is",temp_hum_score )
                    # if temp_hum_score> 1000:
                    #     print("greater than 1000 is", (i, j))
                else:
                    continue
                    # 空白直接跳过
        return comp - hum
